...
def handle_duplicates_within_case(df, cols_boolean, cols_rest):
    ...
    ret = ret.append(df[cols_boolean].any(axis=0))
    return ret
...
def process_quarters(quarters, dir_in, dir_out, config_items, drug_names, reaction_types):
    DEBUG = None
    template_drug = os.path.join(dir_in, 'drugQ.csv.zip')
    usecols = ['primaryid', 'caseid', 'drugname']
    df_drug = load_quarder_files(template_drug, quarters, usecols=usecols, nrows=DEBUG).dropna()
