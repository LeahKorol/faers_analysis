...
def generate_individual_report(config, fn_in, dir_reports, alpha, smoothing, title_in_figure=True):
    ...
    df_summary = summary_table(contingency_matrices=df_contingency, alpha=alpha, smoothing=smoothing)
...
def main(
    ...
    try:
        ...
        final_report = pd.DataFrame([
            tbl.sort_values('q').iloc[-1][columns] for _, tbl in sorted(results.items())
        ]).sort_values('ROR').reset_index(drop=True)
